...
class BlogForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "featured_image": widgets.FileInput(
                attrs={"class": "form-control", "accept": "image/*"}
            ),
            "content": widgets.Textarea(
                attrs={"class": "required form-control", "placeholder": "Content"}
            ),
            "video_url": widgets.URLInput(
                attrs={"class": "required form-control", "placeholder": "Video Link"}
            ),
            "assign_to": widgets.SelectMultiple(
...
class RegistrationForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "phone_number": widgets.NumberInput(
                attrs={"class": "required form-control", "placeholder": "Phone Number"}
            ),
            "whatsapp_number": widgets.TextInput(
                attrs={"class": "form-control", "placeholder": " Whatsapp Number"}
            ),
            "place": widgets.TextInput(
                attrs={"class": "required form-control", "placeholder": "Place"}
            ),
    ...
    def clean_username(self):
        ...
        if User.objects.exclude(pk=self.instance.pk).filter(username=username).exists():
            raise forms.ValidationError(
                'Phone Number "%s" is already in use.' % username
            )
        return username
